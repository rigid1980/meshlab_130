...
*                                                                           *
****************************************************************************/
#include "multiDocViewer_Container.h"
#include "glarea.h"
#include <QMouseEvent>
...
MultiDocViewer_Container *Splitter::getRootContainer()
{
	Splitter * parentSplitter = qobject_cast<Splitter *>(this);
	MultiDocViewer_Container* mvc = qobject_cast<MultiDocViewer_Container *>(parentSplitter);
	while(!mvc)
	{
		parentSplitter = qobject_cast<Splitter *>(parentSplitter->parent());
		mvc= qobject_cast<MultiDocViewer_Container *>(parentSplitter);
	}
	return mvc;
}
*/
MultiDocViewer_Container::MultiDocViewer_Container(QWidget *parent)
: Splitter(parent)
{
...
int MultiDocViewer_Container::getNextViewerId(){
	int newId=-1;
...
void MultiDocViewer_Container::addView(GLArea* viewer,Qt::Orientation orient){
...
void MultiDocViewer_Container::removeView(int viewerId)
{
  GLArea* viewer;
...
void MultiDocViewer_Container::updateCurrent(int current){
...
GLArea * MultiDocViewer_Container::getViewer(int id)
{
...
int MultiDocViewer_Container::getViewerByPicking(QPoint p){
  foreach ( GLArea* viewer, viewerList){
...
GLArea* MultiDocViewer_Container::currentView(){
	return getViewer(currentId);
}

int MultiDocViewer_Container::viewerCounter(){
  return viewerList.count();
}

void MultiDocViewer_Container::updateAllViewer(){
  foreach ( GLArea* viewer, viewerList)
		viewer->update();
}

void MultiDocViewer_Container::resetAllTrackBall(){
  foreach ( GLArea* viewer, viewerList)
    viewer->resetTrackBall();
}

void MultiDocViewer_Container::update(int id){
	getViewer(id)->update();
}

void MultiDocViewer_Container::updateTrackballInViewers()
{
...
void MultiDocViewer_Container::closeEvent( QCloseEvent *event )
{
	bool close = true;
	int ii = 0;
